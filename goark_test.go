...
func TestGoarkFacade_whenRegisteringGeneratedStyleProviders_shouldResolveBeans(t *testing.T) {
	...
	if err := goark.Register[*facadeService](app, "service", func(ctx context.Context,
		resolver goark.Resolver) (*facadeService, error) {
...
func TestGoarkFacade_whenUsingBeanMetadataOptions_shouldResolveByFacadeOptions(t *testing.T) {
	app := goark.MustNew()
	if err := goark.Register[facadeWorker](app, "secondary", func(context.Context,
		goark.Resolver) (facadeWorker, error) {
		return facadeSecondaryWorker{}, nil
	}, goark.WithPriority(100)); err != nil {
		t.Fatalf("register secondary worker failed: %v", err)
	}
	if err := goark.Register[facadeWorker](app, "primary", func(context.Context, goark.Resolver) (
		facadeWorker, error) {
	...
	qualified, err := goark.GetByType[facadeWorker](
		context.Background(),
		app,
		goark.WithQualifier("secondary"),
	)
	if err != nil {
...
func TestGoarkFacade_whenAllowCircularReferencesEnabled_shouldResolveFieldCycle(t *testing.T) {
	...
	if err := goark.Register[*facadeCircularService](app, "service", func(context.Context,
		goark.Resolver) (*facadeCircularService, error) {
		return &facadeCircularService{}, nil
	},
		goark.WithInjectionDependencies("repository"),
		goark.WithTypedDependencyInjector(func(ctx context.Context, resolver goark.Resolver,
			service *facadeCircularService) error {
	...
	if err := goark.Register[*facadeCircularRepository](app, "repository", func(context.Context,
		goark.Resolver) (*facadeCircularRepository, error) {
		return &facadeCircularRepository{}, nil
	},
		goark.WithInjectionDependencies("service"),
		goark.WithTypedDependencyInjector(func(ctx context.Context, resolver goark.Resolver,
			repository *facadeCircularRepository) error {
