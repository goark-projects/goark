	...
	server := httptest.NewServer(
		http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
			...
			if request.URL.Query().Get("view") != "full" ||
				request.URL.Query().Get("trace") != "true" {
				failServer(serverErrors, writer, "query = %q", request.URL.RawQuery)
				return
			}
			if request.Header.Get("X-App") != "goark" ||
				request.Header.Get("X-Trace") != "trace-1" {
...
func TestResponseEnsureSuccessReturnsStatusError(t *testing.T) {
	...
	if statusErr.StatusCode != http.StatusNotFound ||
		!strings.Contains(string(statusErr.Body), "missing") {
...
func TestWithTimeoutCopiesHTTPClient(t *testing.T) {
	...
	client, err := webclient.New(
		webclient.WithHTTPClient(base),
		webclient.WithTimeout(2*time.Second),
	)
	if err != nil {
		t.Fatalf("client new failed: %v", err)
	}
