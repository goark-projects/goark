...
func TestClientPostMultipartBody(t *testing.T) {
	...
	server := httptest.NewServer(
		http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
			...
			if header.Filename != "profile.txt" || string(data) != "hello" ||
				header.Header.Get("Content-Type") != "text/plain" {
				failServer(
					serverErrors,
					writer,
					"file = %q %q %q",
	...
	if response.StatusCode() != http.StatusCreated || response.BodyString() != "uploaded" {
		t.Fatalf(
			"response = %d %q, want 201 uploaded",
			response.StatusCode(),
			response.BodyString(),
		)
	}
...
func TestClientRejectsInvalidMultipartBody(t *testing.T) {
	...
	_, err = client.NewRequest(
		...
		http.MethodPost,
		"http://example.com",
		webclient.WithMultipartFields(nil, webclient.MultipartFile{}),
	)
